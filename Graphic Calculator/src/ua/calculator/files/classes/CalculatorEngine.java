...
public class CalculatorEngine implements ActionListener, FocusListener {    // "Движок" Калькулятора, ActionListener,
    ...
    public void count(String text) {
        ...
        if (currentResult == (double) (long) currentResult) displayText = "" + (long) currentResult;   // Если целое
        else displayText = "" + currentResult;                                                         // Если не целое
     ...
     *
     * @param exception ошибка (исключение), возникшее во время run-time'а. Обязательный параметр, по нему определяется текст ошибки (исключения)
     * @see CustomException
     */
